...
CREATE TABLE PHOTO(
    ...
    RES_PHOTO VARCHAR2(300) NOT NULL,
...
-- 식당 삭제를 위한 제약조건 변경
ALTER TABLE PICK ADD CONSTRAINT PICK_RES_CODE_FK FOREIGN KEY(RES_CODE) REFERENCES RESTAURANT ON DELETE CASCADE;
ALTER TABLE RESERVATION ADD CONSTRAINT RESERVATION_RES_CODE_FK FOREIGN KEY(RES_CODE) REFERENCES RESTAURANT ON DELETE CASCADE;
ALTER TABLE REVIEW ADD CONSTRAINT REVIEW_RES_CODE_FK FOREIGN KEY(RES_CODE) REFERENCES RESTAURANT ON DELETE CASCADE;
ALTER TABLE MENU ADD CONSTRAINT MENU_RES_CODE_FK FOREIGN KEY(RES_CODE) REFERENCES RESTAURANT ON DELETE CASCADE;
ALTER TABLE DISCOUNT ADD CONSTRAINT DISCOUNT_RES_CODE_FK FOREIGN KEY(RES_CODE) REFERENCES RESTAURANT(RES_CODE) ON DELETE CASCADE;
ALTER TABLE PHOTO ADD CONSTRAINT PHOTO_RES_CODE_FK FOREIGN KEY(RES_CODE) REFERENCES RESTAURANT(RES_CODE) ON DELETE CASCADE;


-- 제약조건 삭제
ALTER TABLE PICK DROP CONSTRAINT PICK_RES_CODE_FK;
ALTER TABLE RESERVATION DROP CONSTRAINT RESERVATION_RES_CODE_FK;
ALTER TABLE MENU DROP CONSTRAINT MENU_RES_CODE_FK;
ALTER TABLE REVIEW DROP CONSTRAINT REVIEW_RES_CODE_FK;
ALTER TABLE DISCOUNT DROP CONSTRAINT DISCOUNT_RES_CODE_FK;
ALTER TABLE PHOTO DROP CONSTRAINT PHOTO_RES_CODE_FK;
...
ALTER SESSION SET NLS_TIMESTAMP_FORMAT='HH24:MM';

SELECT * FROM restaurant WHERE id='OwnerTest3';
SELECT * FROM restaurant WHERE res_Name LIKE '%식당%';

SELECT DISTINCT R.RES_CODE, R.RES_NAME
FROM RESTAURANT R
...
— 리뷰 정렬
— (평점 높은순)
SELECT * FROM REVIEW ORDER BY review_grade DESC;
— (평점 낮은순)
SELECT * FROM REVIEW ORDER BY review_grade;
— (최신순)
SELECT * FROM REVIEW ORDER BY review_date DESC;

— 평점이 ?인 리뷰 조회
SELECT * FROM REVIEW WHERE review_grade = 4 ORDER BY review_grade DESC;